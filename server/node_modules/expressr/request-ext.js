...
module.exports = exports = {
   ...
   *
   * Default value is coerced to array.
   *
   * @param name
   * @param defaultValue
   ...
   *
   * Default value is coerced to array and filtered.
   *
   * @param name
   * @param defaultValue
  ...
  getNumber: function(name, defaultValue) {
    ...
    return isNaN(value) ? parseFloat(defaultValue) || 0 : value;
  },
   ...
   * in `YYYY-MM-DD` format (UTC, start of day).
   *
  ...
  getMoment: function(name, defaultValue) {
    var d = moment.utc(this.getString(name), 'YYYY-MM-DD').startOf('day');
    return d.isValid() ? d : moment.utc(defaultValue, 'YYYY-MM-DD').startOf('day');
  },
  ...
  getMoments: function(name, defaultValue) {
    ...
    if (Array.isArray(value))
      return flatMap(value, function(v) {
        var mom = moment.utc(v, 'YYYY-MM-DD').startOf('day');
        return mom.isValid() ? mom : null;
      });
    // Single value
    var mom = moment.utc(value, 'YYYY-MM-DD').startOf('day');
